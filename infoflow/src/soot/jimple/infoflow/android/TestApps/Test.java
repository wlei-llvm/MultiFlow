...
public class Test {
	...
	private static boolean parseAdditionalOptions(String[] args) {
		...
		while (i < args.length) {
			...
			//MULTI_TAG
			else if (args[i].equalsIgnoreCase("--multimode")) {
				String algo = args[i+1];
				if (algo.equalsIgnoreCase("CUSTOM"))
					config.setMultiTaintMode(InfoflowConfiguration.MultiTaintMode.Custom);
				else if (algo.equalsIgnoreCase("CUSTOMALL"))
					config.setMultiTaintMode(InfoflowConfiguration.MultiTaintMode.CustomALL);
				else if (algo.equalsIgnoreCase("ALLCOMB"))
					config.setMultiTaintMode(InfoflowConfiguration.MultiTaintMode.AllCombination);
				else if (algo.equalsIgnoreCase("ALLCOMBCALLBACK"))
					config.setMultiTaintMode(InfoflowConfiguration.MultiTaintMode.AllCombCallBackPartition);
				else if (algo.equalsIgnoreCase("SOLO"))
					config.setMultiTaintMode(InfoflowConfiguration.MultiTaintMode.SoloModeForTest);
				else {
					System.err.println("Invalid taint mode ,Valid mode : [\"CUSTOM\"}");
					return false;
				}
				i += 2;
			}
			else if (args[i].equalsIgnoreCase("--callbackend")) {
				config.setEnableCallBackEnd(true);
				i++;
			}
			else if (args[i].equalsIgnoreCase("--nocallbackssource")) {
				config.setEnableCallbackSources(false);
				i++;
			}
			else if (args[i].equalsIgnoreCase("--dumpsource")) {
				config.setDumpSourceEnable(true);
...
//				"--repeatCount", Integer.toString(repeatCount),
	...
	private static InfoflowResults runAnalysis(final String fileName, final String androidJar) {
		try {
			...
			//Multi-TAG
			config.setMultiTaintAnalysisEnable(true);

			final InfoflowResults res;
			if(config.getMultiTaintAnalysisEnable()){
				System.out.println("Running multiple sources and sinks data flow analysis...");
				res = app.runMultiInfoflow(new MyResultsAvailableHandler());
			}else {

				System.out.println("Running data flow analysis...");
				res = app.runInfoflow(new MyResultsAvailableHandler());
			}
			System.out.println("MultiAnalysis has run for " + (System.nanoTime() - beforeRun) / 1E9 + " seconds");
