...
public class IFDSSolver<N,D extends FastSolverLinkedNode<D, N>,M,I extends BiDiInterproceduralCFG<N, M>> {
						...
						for(N retSiteN: returnSiteNs) {
							...
							for(D d5: computeReturnFlowFunction(retFunction, d3, d4, n, Collections.singleton(d1))) {
								...
								if (d5.equals(d2))
									...
									//Multi-Tag
									d5p.setSummaryPredecessor(new Pair<Triplet<N, N, D>, D>(new Triplet<N, N, D>(n, retSiteN, d3), d4));
								}
					...
					for(Entry<D, D> d1d2entry : entry.getValue().entrySet()) {
						...
						for(D d5: targets) {
							...
							if (d5.equals(predVal))
								...
								D d5pClone = d5p.clone();
								d5pClone.setPredecessor(predVal);
								//Multi-Tag
								d5pClone.setSummaryPredecessor(new Pair<Triplet<N, N, D>, D>(new Triplet<N, N, D>(c, retSiteC, d1), d2));
								storeEndAbs(d5pClone, d5p);
								d5p = d5pClone;
							}
	...
	protected void storeEndAbs(D end , D predecessor) {
		return ;
	}
