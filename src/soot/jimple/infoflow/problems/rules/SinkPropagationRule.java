...
public class SinkPropagationRule extends AbstractTaintPropagationRule {
	...
	public Collection<Abstraction> propagateNormalFlow(Abstraction d1,
		...
		if (stmt instanceof ReturnStmt) {
			...
			return checkForSink(d1, source, stmt, returnStmt.getOp());
		}
		else if (stmt instanceof IfStmt) {
			final IfStmt ifStmt = (IfStmt) stmt;
			return checkForSink(d1, source, stmt, ifStmt.getCondition());
		}
		else if (stmt instanceof LookupSwitchStmt) {
			final LookupSwitchStmt switchStmt = (LookupSwitchStmt) stmt;
			return checkForSink(d1, source, stmt, switchStmt.getKey());
		}
			...
			return checkForSink(d1, source, stmt, assignStmt.getRightOp());
		}
	 ...
	 */
	private Collection<Abstraction> checkForSink(Abstraction d1, Abstraction source, Stmt stmt,
			final Value retVal) {
		...
		for (Value val : BaseSelector.selectBaseList(retVal, false)) {
			if (getManager().getSourceSinkManager() != null
				...
				if(!killState) {
					Abstraction endAbs = AbstractionVector.getEndAbstraction();
					endAbs.setEndAbstraction(source);
					endAbs.setEndSinkStmt(stmt);
					endAbs.setSourceContext(source.getSourceContext());
					getResults().addEndAbs(endAbs);
					return Collections.singleton(endAbs);
				}
			}
		...
		if (stmt instanceof ReturnStmt) {
			...
			if (matches
				...
				if(!killState) {
					...
					Abstraction endAbs = AbstractionVector.getEndAbstraction();
					endAbs.setEndAbstraction(source);
					endAbs.setEndSinkStmt(returnStmt);
					endAbs.setSourceContext(source.getSourceContext());
					getResults().addEndAbs(endAbs);
					return Collections.singleton(endAbs);
				}
