...
public class SetupApplication {
	...
	public final static String CUSTOM_SOURCE_FILE  = "CustomSourcesAndSinks.txt";
	...
	public void calculateSourcesSinksEntrypoints(ISourceSinkDefinitionProvider sourcesAndSinks)
		...
		// Create the SourceSinkManager
		{
			...
			if(config.getMultiTaintAnalysisEnable() && (config.getMultiTaintMode() == InfoflowConfiguration.MultiTaintMode.CustomALL ||
					config.getMultiTaintMode() == InfoflowConfiguration.MultiTaintMode.Custom)) {
				//ret set the sources and sinks in multi running.
				ISourceSinkDefinitionProvider customSourceSink = PermissionMethodParser.fromFile(CUSTOM_SOURCE_FILE);
				sourceSinkManager.reSetSources(customSourceSink.getSources());
			}
		}
	 ...
	 * @return The results of the data flow analysis
	 */
	public InfoflowResults runMultiInfoflow(ResultsAvailableHandler onResultsAvailable) {
		if (this.sourceSinkProvider == null)
			throw new RuntimeException("Sources and/or sinks not calculated yet");

		System.out.println("Running data flow analysis on " + apkFileLocation + " with " + getSources().size()
				+ " sources and " + getSinks().size() + " sinks...");
		MultiInfoflow multiInfo;
		if (cfgFactory == null)
			multiInfo = new MultiInfoflow(androidJar, forceAndroidJar, null,
					new DefaultPathBuilderFactory(config.getPathBuilder(),
							config.getComputeResultPaths()));
		else
			multiInfo = new MultiInfoflow(androidJar, forceAndroidJar, cfgFactory,
					new DefaultPathBuilderFactory(config.getPathBuilder(),
							config.getComputeResultPaths()));
		...
		multiInfo.setTaintWrapper(taintWrapper);
		if (onResultsAvailable != null)
			multiInfo.addResultsAvailableHandler(onResultsAvailable);

		System.out.println("Starting infoflow computation...");
		multiInfo.setConfig(config);
		multiInfo.setSootConfig(sootConfig);

		if (null != ipcManager) {
			multiInfo.setIPCManager(ipcManager);
		}

		multiInfo.computeInfoflow(apkFileLocation, path, entryPointCreator, sourceSinkManager);
		this.maxMemoryConsumption = multiInfo.getMaxMemoryConsumption();
		this.collectedSources = multiInfo.getCollectedSources();
		this.collectedSinks = multiInfo.getCollectedSinks();

		return multiInfo.getResults();
	}
